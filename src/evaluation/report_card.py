...
def write_benchmark_report(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    if not rows:
        ...
        lines.append("")
        lines.append("| task_id | e2e | code | completeness | coverage | quality | consistency | sanity | figures | repairs | runtime_seconds |")
        lines.append("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
        for row in rows:
            lines.append(
                f"| {row.get('task_id')} | {row.get('end_to_end_success')} | "
                f"{row.get('code_execution_success')} | {row.get('report_completeness_score')} | "
                f"{row.get('question_coverage_score')} | {row.get('report_quality_score')} | "
                f"{row.get('consistency_score')} | {row.get('result_sanity_score')} | {row.get('figure_count')} | "
